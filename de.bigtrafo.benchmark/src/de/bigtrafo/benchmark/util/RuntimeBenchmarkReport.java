...
public class RuntimeBenchmarkReport {
	...
	public void addEntryHeader(String id) {
		count++;
		StringBuilder info = new StringBuilder();
		info.append('\n');
	...
	public void addSubEntry(Unit unit, int before, int after, long runtime) {
		count++;
		StringBuilder info = new StringBuilder();
		info.append("   Applied unit "+unit.getName()+" in "+sec(runtime) + " sec, ");
		info.append("change: " + before + " nodes -> " + after + " nodes (delta = "
				+ (after - before) + ")\n");
		addToReport(info);
	}
	...
	public void finishEntry(int before, int after, long runtime) {
		...
		info.append("Performed change: " + before + " nodes -> " + after + " nodes (delta = "
				+ (after - before) + ")\n");
		addToReport(info);
	}
